...
type IUserRepository interface {
	Login(user *sys.User) (*sys.User, error)           // 登录
	ChangePwd(username string, newPasswd string) error // 更新密码

	CreateUser(user *sys.User) error                                // 创建用户
	GetUserById(id uint) (sys.User, error)                          // 获取单个用户
	GetUsers(req *bean.UserListRequest) ([]*sys.User, int64, error) // 获取用户列表
	UpdateUser(user *sys.User) error                                // 更新用户
	BatchDeleteUserByIds(ids []uint) error                          // 批量删除

	GetCurrentUser(c *gin.Context) (sys.User, error)                  // 获取当前登录用户信息
	GetCurrentUserMinRoleSort(c *gin.Context) (uint, sys.User, error) // 获取当前用户角色排序最小值（最高等级角色）以及当前用户信息
	GetUserMinRoleSortsByIds(ids []uint) ([]int, error)               // 根据用户ID获取用户角色排序最小值

	SetUserInfoCache(username string, user sys.User) // 设置用户信息缓存
...
func (ur UserRepository) UpdateUser(user *sys.User) error {
	...
	err = config.DB.Model(user).Association("Roles").Replace(user.Roles)
...
func (ur UserRepository) BatchDeleteUserByIds(ids []uint) error {
	...
	err := config.DB.Select("Roles").Unscoped().Delete(&users).Error
	// 删除用户成功，则删除用户信息缓存
	if err == nil {
