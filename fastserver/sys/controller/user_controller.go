...
func (uc UserController) GetUserInfo(c *gin.Context) {
	...
	if err != nil {
		Fail(c, nil, "获取当前用户信息失败: "+err.Error())
		return
	}
...
func (uc UserController) GetUsers(c *gin.Context) {
	...
	Success(c, gin.H{"users": ToUsersDto(users), "total": total}, "获取用户列表成功")
}
...
func (uc UserController) ChangePwd(c *gin.Context) {
	...
	if err != nil {
		Fail(c, nil, "更新密码失败: "+err.Error())
		return
	}
	Success(c, nil, "更新密码成功")
}
...
func (uc UserController) CreateUser(c *gin.Context) {
	...
	// 根据角色id获取角色
	rr := service.NewRoleDao()
	roles, err := rr.GetRolesByIds(reqRoleIds)
	if err != nil {
		Fail(c, nil, "根据角色ID获取角色信息失败: "+err.Error())
		return
	}
	if len(roles) == 0 {
		Fail(c, nil, "未获取到角色信息")
	...
	if currentRoleSortMin >= reqRoleSortMin {
		Fail(c, nil, "用户不能创建比自己等级高的或者相同等级的用户")
		return
	}
	...
	if err != nil {
		Fail(c, nil, "创建用户失败: "+err.Error())
		return
	}
	Success(c, nil, "创建用户成功")
...
func (uc UserController) UpdateUserById(c *gin.Context) {
	...
	if err != nil {
		Fail(c, nil, "获取需要更新的用户信息失败: "+err.Error())
		return
	}
	...
	if userId == int(ctxUser.ID) {
		...
		if len(reqDiff.([]uint)) > 0 || len(currentDiff.([]uint)) > 0 {
			Fail(c, nil, "不能更改自己的角色")
			return
		}

		// 不能更新自己的密码，只能在个人中心更新
		if req.Password != "" {
			Fail(c, nil, "请到个人中心更新自身密码")
			return
		}
		...
		if err != nil || len(minRoleSorts) == 0 {
			Fail(c, nil, "根据用户ID获取用户角色排序最小值失败")
			return
		}
		if currentRoleSortMin >= minRoleSorts[0] {
			Fail(c, nil, "用户不能更新比自己角色等级高的或者相同等级的用户")
			return
		}

		// 用户不能把别的用户角色等级更新得比自己高或相等
		if currentRoleSortMin >= reqRoleSortMin {
			Fail(c, nil, "用户不能把别的用户角色等级更新得比自己高或相等")
			return
		}
	...
	if err != nil {
		Fail(c, nil, "更新用户失败: "+err.Error())
		return
	}
	Success(c, nil, "更新用户成功")
...
func (uc UserController) BatchDeleteUserByIds(c *gin.Context) {
	...
	if err != nil {
		Fail(c, nil, "删除用户失败: "+err.Error())
		return
	}

	Success(c, nil, "删除用户成功")
