...
func (rc *RoleController) CreateRole(c *gin.Context) {
	...
	if err != nil {
		util.ServerError(c, "获取当前用户最高角色等级失败: "+err.Error())
		return
	}
	if sort >= req.Sort {
		util.ServerError(c, "不能创建比自己等级高或相同等级的角色")
		return
	}
	role := model.Role{
		Name:    req.Name,
	...
	if err != nil {
		util.ServerError(c, "创建角色失败: "+err.Error())
		return
	}
	util.Success(c, nil)
}
...
func (rc *RoleController) Update(c *gin.Context) {
	...
	if minSort >= roles[0].Sort {
		util.ServerError(c, "不能更新比自己角色等级高或相等的角色")
		return
	}
	if minSort >= req.Sort {
		util.ServerError(c, "不能把角色等级更新得比当前用户的等级高或相同")
		return
	}
	role := model.Role{
		Name:    req.Name,
	...
	if req.Keyword != roles[0].Keyword {
		...
		if !isAdded {
			util.ServerError(c, "更新角色成功，但角色关键字关联的权限接口更新失败")
			return
		}
		...
		if err != nil {
			util.ServerError(c, "更新角色成功，但角色关键字关联角色的权限接口策略加载失败")
			return
		}
	}
...
func (rc *RoleController) GetRoleMenusById(c *gin.Context) {
	...
	if err != nil {
		util.ServerError(c, "获取角色的权限菜单失败: "+err.Error())
		return
	}
...
func (rc *RoleController) UpdateRoleMenusById(c *gin.Context) {
	...
	if minSort != 1 {
		if minSort >= roles[0].Sort {
			util.ServerError(c, "不能更新比自己角色等级高或相等角色的权限菜单")
			return
		}
	}
	...
	if err != nil {
		util.ServerError(c, "获取当前用户的可访问菜单列表失败: "+err.Error())
		return
	}
	...
	if minSort != 1 {
		for _, id := range menuIds {
			if !slices.Contains(ctxUserMenusIds, id) {
				util.ServerError(c, fmt.Sprintf("无权设置ID为%d的菜单", id))
				return
			}
		}
	...
	if err != nil {
		util.ServerError(c, "更新角色的权限菜单失败: "+err.Error())
		return
	}
...
func (rc *RoleController) UpdateRoleApisById(c *gin.Context) {
	...
	if minSort != 1 {
		if minSort >= roles[0].Sort {
			util.ServerError(c, "不能更新比自己角色等级高或相等角色的权限接口")
			return
		}
	}
	...
	for _, role := range ctxRoles {
		...
		if err2 != nil {
			util.ServerError(c, "获取当前用户的角色关键字关联的权限接口失败")
			return
		}
	...
	if err != nil {
		util.ServerError(c, "根据接口ID获取接口信息失败")
		return
	}
	...
	if minSort != 1 {
		for _, reqPolicy := range reqRolePolicies {
			if !funk.Contains(ctxRolesPolicies, reqPolicy) {
				util.ServerError(c, fmt.Sprintf("无权设置路径为%s,请求方式为%s的接口", reqPolicy[1], reqPolicy[2]))
				return
			}
		}
	}
...
func (rc *RoleController) BatchDeleteRoleByIds(c *gin.Context) {
	...
	if err != nil {
		util.ServerError(c, "获取角色信息失败: "+err.Error())
		return
	}
	if len(roles) == 0 {
		util.ServerError(c, "未获取到角色信息")
	...
	for _, role := range roles {
		if minSort >= role.Sort {
			util.ServerError(c, "不能删除比自己角色等级高或相等的角色")
			return
		}
	}
	err = rc.roleService.BatchDeleteRoleByIds(roleIds)
	if err != nil {
		util.ServerError(c, "删除角色失败")
		return
	}
	rc.userService.ClearUserInfoCache()
	util.Success(c, nil)
