...
// 获取角色列表
func (r RoleService) GetRoles(req *request.RoleListRequest) ([]sys.Role, int64, error) {
	return r.roleDao.GetRoles(req)
}

// 根据角色ID获取角色
func (r RoleService) GetRolesByIds(roleIds []uint) ([]*sys.Role, error) {
	return r.roleDao.GetRolesByIds(roleIds)
}

// 创建角色
func (r RoleService) CreateRole(role *sys.Role) error {
	return r.roleDao.CreateRole(role)
}

// 更新角色
func (r RoleService) UpdateRoleById(roleId uint, role *sys.Role) error {
	return r.roleDao.UpdateRoleById(roleId, role)
}

// 获取角色的权限菜单
func (r RoleService) GetRoleMenusById(roleId uint) ([]*sys.Menu, error) {
	return r.roleDao.GetRoleMenusById(roleId)
}

// 更新角色的权限菜单
func (r RoleService) UpdateRoleMenus(role *sys.Role) error {
	return r.roleDao.UpdateRoleMenus(role)
}

// 根据角色关键字获取角色的权限接口
func (r RoleService) GetRoleApisByRoleKeyword(roleKeyword string) ([]*sys.Api, error) {
